...
func CreateUser(user *models.User) (msg string, err error) {
	...
	insertQuery := "INSERT INTO users (id, firstname, lastname, email, password) VALUES (?, ?, ?, ?, ?)"
...
func CheckUserExistsByEmail(email string) bool {
	userExistsQuery := "SELECT 1 FROM users WHERE email = ?"
	var exists bool
...
func GetUserByUsername(email string) (user *models.User, err error) {
	user = &models.User{}
	userExistsQuery := "SELECT id,firstname,lastname,email,password FROM users WHERE email = ?"
...
func CreateTask(task *models.Tasks) (string, error) {
	...
	insertQuery := "INSERT INTO tasks (id, title, description, user_id, priority, due_date, completed, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
...
func GetTasks(uid string) ([]*models.Tasks, error) {
	...
	tasks := []*models.Tasks{}
	query := "SELECT id, title, description, user_id, priority, due_date, completed, completed_at, created_at FROM tasks WHERE user_id = ?"
